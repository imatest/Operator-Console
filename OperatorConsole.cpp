...
BOOL COperatorConsoleApp::InitInstance()
{
	...
	if (!InitLibs()) // InitLibs must be called before any calls to Imatest library functions
	{
		...
		SEVERE_LOG(logger, "Unable to initialize the Imatest library.");
	}
	...
	if (!ReadINISettings()) // if we're unable to read the ini file, we must generate image dimensions from user input before continuing
	{
		...
		WARNING_LOG(logger, "INI settings for Operator Console not found. Please click the 'Setup' button and fill in the appropriate fields before proceeding.");

	}
	if (!ReadPassFail())
	{
		WARNING_LOG(logger, "Unable to read the pass/fail file.")
	}
	if (!Init())	// do our application-specific initialization
	{
		cout << "Application Initialization Failed" << endl;
		SEVERE_LOG(logger, "Application Initialization Failed");
...
bool COperatorConsoleApp::Init()
{
	...
	FINEST_LOG(logger, "Entering COperatorConsoleApp::Init().");
	...
	if (!errMsg.IsEmpty())
	{
		SEVERE_LOG(logger, errMsg);
		AfxMessageBox(errMsg);
	}
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::Init().");
	return success;
}
...
{
	bool	success = false;
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitBlemishAcq()")
	if (m_blemishAcq.Init(RAW_WIDTH_BLEMISH, RAW_HEIGHT_BLEMISH, RAW_BLEMISH_IMAGE))
	{
		FINE_LOG(logger, "m_blemishAcq.Init succeeded.")
		success = m_blemishAcq.Open();
	}
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitBlemishAcq()")
	return success;
}
...
{
	bool	success = false;
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitSFRplusAcq()")
	if (m_sfrPlusAcq.Init(RAW_WIDTH_SFRPLUS, RAW_HEIGHT_SFRPLUS, RAW_SFRPLUS_IMAGE))
	{
		success = m_sfrPlusAcq.Open();
	}
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitSFRplusAcq()")
	return success;
}
...
{
	bool	success = false;
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitCamera()");

	if (m_image_source == imatest_source)
	{
		FINE_LOG(logger, "imatest_source is selected.");
	...
	else if (m_image_source == file_source)
	{
		...
		//success = m_camera->Open();
		FINE_LOG(logger, "file_source is selected.");
		cerr << "Error! Image file loading not yet implemented." << endl;
		SEVERE_LOG(logger, "Error! Image file loading not yet implemented.");
	}
	...
	//	success = m_camera.Open();
	//#endif
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitCamera().");
	return success;
}

bool COperatorConsoleApp::InitBlemishThread()
{
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitBlemishThread()");
#if defined USE_RGB_DATA
	FINE_LOG(logger, "Initializing m_blemish with m_cameraImage.");
	m_blemish.Init(m_cameraImage, m_setup.width, m_setup.height, m_config);

#else
	FINE_LOG(logger, "Initializing m_blemish with m_fileImage.");
	m_blemish.Init(m_fileImage, m_blemishAcq.GetWidth(), m_blemishAcq.GetHeight(), m_config);
#endif

	m_flags.blemishThread = m_blemishControl.Init(MSG_BLEMISH_DONE, m_nThreadID, m_blemish.ThreadProc, &m_blemish);

	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitBlemishThread().");
...
{
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitSFRplusThread()");
#if defined USE_RGB_DATA
	FINE_LOG(logger, "Initializing m_sfrPlus with m_cameraImage.");
	m_sfrPlus.Init(m_cameraImage, m_setup.width, m_setup.height, m_config);
#else
	FINE_LOG(logger, "Initializing m_sfrPlus with m_fileImage.");
	m_sfrPlus.Init(m_fileImage, m_sfrPlusAcq.GetWidth(), m_sfrPlusAcq.GetHeight(), m_config);
#endif
	m_flags.sfrplusThread = m_sfrPlusControl.Init(MSG_SFRPLUS_DONE, m_nThreadID, m_sfrPlus.ThreadProc, &m_sfrPlus);

	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitSFRplusThread().");
...
{
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitLibs().");
	CString	str;
	...
	if (!m_flags.matlab)
	{
		SEVERE_LOG(logger, "mclInitializeApplication() call failed.");
	...
	else
	{
		...
		//
		FINE_LOG(logger, "mclInitializeApplication() call succeeded.");
	...
	if (!str.IsEmpty())
	{
		SEVERE_LOG(logger, str);
		AfxMessageBox(str);
	}

	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitLibs().");
	return m_flags.matlab && m_flags.imatestIT && m_flags.imatestAcq;
}

void COperatorConsoleApp::CloseLibs()
{
	FINEST_LOG(logger, "Entering COperatorConsoleApp::CloseLibs().");
	if (m_flags.imatestIT)
	{
		FINE_LOG(logger, "Calling imatest_libraryTerminate().");
	...
	if (m_flags.imatestAcq)
	{
		FINE_LOG(logger, "Calling imatest_acquisitionTerminate().");
	...
	if (m_flags.matlab)
	{
		FINE_LOG(logger, "Calling mclTerminateApplication().");
		mclTerminateApplication();	// terminate MATLAB runtime
		m_flags.matlab = false;
	}
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::CloseLibs().");
}
...
{
	FINEST_LOG(logger, "Entering COperatorConsoleApp::InitOutput().");
	...
	m_flags.stdOut = true;
	m_flags.stdErr = true;
	FINE_LOG(logger, "STDIO redirection enabled.");
#else
	FINE_LOG(logger, "STDIO redirection disabled.");
#endif
	FINEST_LOG(logger, "Exiting COperatorConsoleApp::InitOutput().");
...
{
	BOOL	handled = FALSE;
	FINEST_LOG(logger, "Entering COperatorConsoleApp::PreTranslateMessage().");

	switch (pMsg->message)
	{
	case MSG_START:
		FINE_LOG(logger, "PreTranslateMessage received MSG_START.");
		OnStart((WPARAM)0, (LPARAM)0);
		handled = TRUE;
		break;

	case MSG_STOP:
		FINE_LOG(logger, "PreTranslateMessage received MSG_STOP.");
		OnStop((WPARAM)0, (LPARAM)0);
		handled = TRUE;
		break;

	case MSG_FRAME_READY:
		FINE_LOG(logger, "PreTranslateMessage received MSG_FRAME_READY.");
		OnFrameReady(pMsg->wParam, pMsg->lParam);
		break;

	case MSG_RUN_TEST:
		FINE_LOG(logger, "PreTranslateMessage received MSG_RUN_TEST.");
		OnRunTest((WPARAM)0, (LPARAM)0);
		handled = TRUE;
		break;

	case MSG_BLEMISH_DONE:
		FINE_LOG(logger, "PreTranslateMessage received MSG_BLEMISH_DONE.");
		OnBlemishDone(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;

	case MSG_SFRPLUS_DONE:
		FINE_LOG(logger, "PreTranslateMessage received MSG_SFRPLUS_DONE.");
		OnSFRplusDone(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;

	case MSG_SET_BLEMISH:
		FINE_LOG(logger, "PreTranslateMessage received MSG_SET_BLEMISH.");
		OnSetBlemish(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;

	case MSG_SET_SFRPLUS:
		FINE_LOG(logger, "PreTranslateMessage received MSG_SET_SFRPLUS.");
		OnSetSFRplus(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;

	case MSG_JSON:
		FINE_LOG(logger, "PreTranslateMessage received MSG_JSON.");
		OnShowJSON(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;

	case MSG_STDOUT:
		FINE_LOG(logger, "PreTranslateMessage received MSG_STDOUT.");
		OnUpdateStdout();
		break;

	case MSG_STDERR:
		FINE_LOG(logger, "PreTranslateMessage received MSG_STDERR.");
		OnUpdateStderr();
		break;

	case MSG_SETUP:
		FINE_LOG(logger, "PreTranslateMessage received MSG_SETUP.");
		OnSetup(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;
	case MSG_PASS_FAIL:
		FINE_LOG(logger, "PreTranslateMessage received MSG_PASS_FAIL.");
		OnPassFail(pMsg->wParam, pMsg->lParam);
		handled = TRUE;
		break;
	case MSG_SET_IMATEST_CAM:
		FINE_LOG(logger, "PreTranslateMessage received MSG_SET_IMATEST_CAM.");
...
bool COperatorConsoleApp::ReadINISettings(void)
{
	...
	// we must have the width and height > 0 since we will be allocating buffers soon
	bool result = m_setup.width > 0 && m_setup.height > 0;
